...
import demo.log.Logger;
import demo.ui.BaseActivity;
import demo.ui.record.views.ProgressView;
import demo.ui.record.views.ThemeRadioButton;
import demo.utils.ConvertToUtils;
...
public class MediaRecorderActivity extends BaseActivity implements OnErrorListener, OnClickListener, OnPreparedListener {
	...
	/** 录制最长时间 */
	public final static int RECORD_TIME_MAX = 10 * 2000;
	/** 录制最小时间 */
    ...
    private void initBradcast(){
        videoBroadcastReceiver = new BroadcastReceiver() {
            ...
            public void onReceive(Context context, Intent intent) {
                System.out.println("选择视频，开始转码");
                videoArray=(intent.getStringArrayListExtra("list"));
                if(videoArray.size()!=0){
                    System.out.println("1");
                    //Uri uri = Uri.fromFile(new File(videoArray.get(0)));
                    System.out.println("uri"+videoArray.get(0));
                    System.out.println("2");
                    String columnName = MediaStore.Video.Media.DATA;
                    System.out.println("columnName"+columnName);
                    System.out.println("4");
                    String path = "";
                    path = videoArray.get(0);
                    if (saveMediaObject(mMediaObject)) {
                        System.out.println("5");
                        startActivityForResult(new Intent(MediaRecorderActivity.this, ImportVideoActivity.class).putExtra("obj", mMediaObject.getObjectFilePath()).putExtra("path", path), REQUEST_CODE_IMPORT_VIDEO_EDIT);
                    }
	...
	private void initMediaRecorder() {
		...
		mMediaRecorder.setVideoBitRate(NetworkUtils.isWifiAvailable(this) ?800 : MediaRecorder.VIDEO_BITRATE_NORMAL);
		//		mMediaRecorder.setSurfaceHolder(mSurfaceView.getHolder());
		mMediaRecorder.setSurfaceView(mSurfaceView);
		String key = String.valueOf(System.currentTimeMillis());
        //Toast.makeText(MediaRecorderActivity.this,"VCamera.getVideoCachePath()"+"  "+VCamera.getVideoCachePath(),Toast.LENGTH_LONG).show();

        File mediaStorageDir = new File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_PICTURES), "learnNcode");
        if (! mediaStorageDir.exists()){
            if (! mediaStorageDir.mkdirs()){
                Toast.makeText(MediaRecorderActivity.this,"文件创建失败",Toast.LENGTH_LONG).show();
            }
        }
        //Toast.makeText(MediaRecorderActivity.this,mediaStorageDir.getPath() + File.separator,Toast.LENGTH_LONG).show();

		mMediaObject = mMediaRecorder.setOutputDirectory(key, VCamera.getVideoCachePath() + key);
         Toast.makeText(MediaRecorderActivity.this,"存放地址参数:"+ VCamera.getVideoCachePath() + key,Toast.LENGTH_LONG).show();
         Toast.makeText(MediaRecorderActivity.this,"存放准确地址:"+  mMediaObject.getOutputVideoPath(),Toast.LENGTH_LONG).show();
        //mMediaObject = mMediaRecorder.setOutputDirectory(key, mediaStorageDir.getPath() + File.separator + key);
		if (mMediaObject != null) {
			mMediaRecorder.prepare();
				...
				protected void onPostExecute(Boolean result) {
					...
					if (result) {
						...
						if (saveMediaObject(mMediaObject)) {
							...
							*/
                            HomeFragmentActivity.fileUri=Uri.fromFile(new File(mMediaObject.getOutputVideoPath()));
                            /*
                            Intent intent1=new Intent();
                            intent1.putExtra("videofile",mMediaObject.getOutputVideoPath());
                            setResult(821821,intent1);
                            */
                            Intent data=new Intent();
                            data.putExtra("videofile",mMediaObject.getOutputVideoPath());
                            setResult(999,data);
                            finish();
						} else {
...
/*
			...
			*/
            ...
            Intent intent = new Intent(MediaRecorderActivity.this, HomeFragmentActivity.class);
