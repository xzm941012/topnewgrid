 ...
 */
public class Activity_choose_huodong extends FragmentActivity implements OnItemClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_choose_pindao_huodong);
        initUser();
        initLayout();
        initListenner();
		initView();
            ...
            case 2:
                ...
                for(ChannelItem channelItem1:userChannelList){
                    if(channelItem1.getName().equals(str)){
                        Toast.makeText(Activity_choose_huodong.this,"频道已存在",Toast.LENGTH_SHORT).show();
                        return ;
                    }
                }
    ...
    private void initLayout(){
        morePindao=(Button)findViewById(R.id.button3);
        next=(TextView)findViewById(R.id.textView61);
    }
	private void initListenner(){
        findViewById(R.id.imageView70).setOnClickListener(new View.OnClickListener() {
            @Override
        ...
        morePindao.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                //startActivityForResult(new Intent(Activity_choose_huodong.this, Activity_pindao_list.class), 1);
            }
        });
        next.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                final StringBuffer titles=new StringBuffer();
                for(ChannelItem channelItem:userChannelList){
                    if(!channelItem.getName().equals("推荐")&&!channelItem.getName().equals("全部")) {
                        titles.append(channelItem.getName() + ";");
                    }
                }
                new Thread(new Runnable() {
                    ...
                    public void run() {
                        try {
                            System.out.println("标签是："+titles.toString());
                            String result=Manage.AlterChannel(titles.toString(),user.getUserId()+"");
                            String[]a=titles.toString().split(";");
